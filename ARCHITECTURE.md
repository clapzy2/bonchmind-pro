...
**Multi-tenant security (Stage 13):** фундамент под B2B. Rate-limit ключуется **по пользователю** для авторизованных ручек (`src/rate_limit.user_or_ip`) — общий NAT вуза больше не позволяет одному студенту задушить группу; `login`/`register` остаются per-IP. Бан (`is_active=false`) теперь проверяется в `get_current_user`, поэтому рубит уже выданный JWT сразу. Superuser управляет пользователями: `GET /api/admin/users`, `POST /api/admin/users/{id}/role|active` (promote/demote, ban/unban) с двумя защитами — self-guard и last-active-superuser guard. Стратегическая модель зафиксирована как **B2B-seat (кафедра)**, B2C — витрина; следующий крупный слой (Stage 15, Organization/Courses/роли + `can(user, action, workspace)`) спроектирован в [`design/multi-tenant-security.md`](design/multi-tenant-security.md).

**Streaming chat (Stage 14):** ответ ассистента печатается по токенам. `POST /api/chat/stream` отдаёт `StreamingResponse` (NDJSON: события `token`, затем финальный `done` с answer/sources/confidence/followups/history). Общая подготовка (классификация сообщения, ретрив, сборка промпта) вынесена в `app_services._prepare_chat`, который делят sync (`chat_service`) и stream (`chat_stream_service`) пути. Квота проверяется в роуте **до** старта потока (402 — обычным JSON), usage пишется только **после** полного ответа (отмена/сбой не списывают; фолбэк фронта на `/api/chat` не задваивает). Конспект **не** стримится. На фронте — растущий ответ с курсором, кнопка «Стоп», отмена через `AbortController`.
