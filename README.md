...
### Защищённый главный админ

Чтобы тебя (главного админа) **никто не мог разжаловать или забанить** — даже другой суперпользователь, — укажи свой login-email в env:

```env
ROOT_ADMIN_EMAIL=you@example.com
```

Этот аккаунт станет «root»: его строка в «Пользователях» помечена **«(защищён)»**, кнопки выключены, а API на demote/ban отвечает `400 protected_admin`. Менять `ROOT_ADMIN_EMAIL` можно только из env (через API его не подделать). Само по себе env прав не выдаёт — повысить аккаунт до superuser нужно как обычно (первый раз — через БД).
