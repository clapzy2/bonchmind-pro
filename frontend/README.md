...
* загрузка, удаление и переиндексация материалов (вкладка **Библиотека**);
* inline-загрузка файла прямо из Конспекта и Ассистента (скрепка);
* генерация конспектов + проверка покрытия источниками;
* экспорт конспекта в `.docx`;
* ассистент по загруженным материалам;
* Markdown-рендеринг ответов и конспектов;
* сохранение результата и истории чата при F5 (sessionStorage, по `user.id`).
...
3. Материал загружается и появляется в Библиотеке без F5.
4. Summary работает по загруженному файлу (текст без техзаголовка).
5. Assistant отвечает по загруженному файлу.
6. Конспект показывает покрытие источников («Проверить источники»).
7. F5 на Конспекте/Ассистенте сохраняет результат.
8. Bob не видит материалы Alice, и наоборот.
```
